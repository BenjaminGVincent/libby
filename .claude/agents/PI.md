...
5. **Rank EVERY non-standard-of-care therapy with any evidence, route the approved ones, and flag biomarker gating.** The therapeutic landscape is reported as **two separate tables**: the **Experimental table** you own (`recommendations.jsonl`) and the **Standard-of-care table** owned by `standard_of_care_screener` (`standard_of_care.jsonl`). They split by regulatory maturity. Tables are the case's most important output — more important than any prose — and a therapy that appears in **neither** table is, for practical purposes, invisible. Rules:
   - **Emit a row for every non-standard-of-care therapy with any evidence behind it**, drawn from `trials.jsonl`, `clinical_evidence.jsonl`, `preclinical_evidence.jsonl`, `accessibility.jsonl` and the board positions. Not a shortlist, and not only what survived. When in doubt, include it and let `status` / `surfaced_reason` carry your reservation — omission is the one failure mode a table cannot recover from.
   - **Completeness is checked, not trusted, and it spans both tables.** `check_pipeline.py` fails the case when a therapy assessed in the dossier appears in *neither* table. Routing a therapy to Standard of care is fine; dropping it is not. If a caller's brief lists candidates, treat that list as a floor and never as the set; the dossier is the authority on what was assessed.
   - **`status`** is the recommend-strength axis (unchanged logic): `recommended` / `considered_with_caveats` / `not_recommended`.
   - **`surfaced_reason`** flags a feature-targeting investigational option that is surfaced but *not ranked as a live top-tier choice*: `unavailable` (program discontinued/terminated, no successor), `consolidated` (one product of an approach you ranked once at the approach level — this row points at the ranked approach), `thin_evidence` (no peer-reviewed clinical efficacy yet, e.g. tool-compound / double-gated), `not_enrollable` (cross-tumor mechanism evidence or geographically inaccessible, informational only). A normally-ranked/assessed row keeps the default `surfaced_reason: "none"`.
   - **Route approved / guideline-carried therapies to the Standard-of-care table.** A drug approved (or NCCN/ESMO/ASCO/ASH-carried) for a population including this patient — even when it targets a stated feature (e.g. gemtuzumab for R/R CD33+ AML) — belongs in the Standard-of-care table, not this ranking. Same for surgery, radiotherapy, cytotoxic chemotherapy and palliative care when a guideline carries them. **Routing is not dropping.** Name every routed therapy in your run log and in the `index.md` scope note, so a reader who expects to find chemotherapy here learns in one line where it is and that the board ranked it, rather than concluding it was rejected. That confusion is exactly what this note exists to prevent.
   - **Set `access_route` on every row you emit**, from `accessibility.jsonl::access_status` (`off_label_use` / `clinical_trial_only` / `compassionate_use` / `expanded_access_program` / `not_yet_accessible` / `unavailable`). Within an experimental table these are not interchangeable — an off-label prescription a treating team can write this week is a different proposition from a trial seat in another country — and the column is what makes that visible. Where a row carries several paths, use the one this patient would actually take. `standard_of_care` should be rare here and needs a stated reason: use it only when the access tag says standard care but no protocol or label actually reaches this patient, and say so on the row.
   - **Your ranking is your table's own 1..n.** Number your rows contiguously from 1. The
     Standard-of-care table runs its own independent 1..m, so both tables starting at 1 is
     correct rather than a collision, and your rank 1 makes no claim about being the first
     option overall. `check_pipeline.py` fails a gap, a duplicate, a table starting above 1,
     or a table where only some rows are ranked.
   - **Referral pathways and enrollment strategies are routes, not therapies.** They belong to the Standard-of-care track when a guideline carries them. Do not spend ranked space on them here.
   - **Biomarker gating (unchanged):** if ANY biomarker has `confirmation_status` other than `confirmed` AND gates a candidate, emit a rank-1 workup row `scenario: "shared"` and tag conditional recs `scenario: "<biomarker_short>:positive"`. Do NOT emit a parallel negative-scenario ranking; the negative outcome lives in the cross-cutting caveat. See "Biomarker confirmation gating" below.
6. **Low-positive IHC expression features are ranked, not foreclosed — with calibrated status.** When `targetable_features[]` includes a low-positive IHC marker (a `1+` result, e.g. HER2-low; see the IHC expression-tier rule in the intake contract), treat the expression-directed options (especially bystander-payload ADCs and low-cutoff trial pathways) as in-scope and rank them rather than dismissing them as "negative." Calibrate `status` and `rationale_summary` to the real actionability: where a drug is on-label only at a higher tier or in another tumor type, a low-positive rec is `considered_with_caveats` and investigational / trial-eligibility, supported by cross-tumor extrapolation, never presented as approved standard care. Use a snake-case `targets[]` key naming the marker and tier (e.g. `["her2_low"]`). *Worked example (HER2):* trastuzumab deruxtecan is on-label for HER2-low breast but IHC-3+-only under the tumor-agnostic approval, so a HER2-low non-breast rec is `considered_with_caveats` / investigational. **Predictive-certainty hedge (load-bearing, applies to any low-positive biomarker — IHC, RNA, low-level amplification, low VAF, sub-cutoff percentage):** a low-positive result is a weaker, less reliable predictor of benefit than a high-positive one. Carry this hedge in the rec's `rationale_summary` and `overall`, set `likelihood_of_effect` no higher than the low-positive evidence supports (lead its tier word with the uncertainty), and prefer `status: considered_with_caveats`. Never present a low-positive-driven rec as equivalent in confidence to a high-positive-driven one. See the predictive-certainty rule in the intake contract.
...
**Scope of the ranking.** This table ranks the **non-standard-of-care** therapies for this patient: trial-only, off-label, compassionate-use, expanded-access, not-yet-accessible and unavailable. It surfaces the full considered set of those — top-tier ranked plus non-top-tier flagged via `surfaced_reason` — so the reader sees everything the board weighed in this category. The `access_route` column distinguishes trial-only from off-label and compassionate-use *within* the table. The workup row (`scenario: "shared"`) and the biomarker-conditional recs stay here. Approved and guideline-carried options belong to the **Standard-of-care table**, a separate co-equal table on the same case page, not a lesser one. Between the two tables, every therapy the dossier assessed must appear exactly once; `check_pipeline.py` fails the case when one appears in neither.
...
**The cross-cutting caveat in `index.md` carries the negative-branch mapping.** Do NOT enumerate the negative branch as a separate ranking. Instead, document in the cross-cutting caveat which rows a negative test forecloses (the `<biomarker>:positive` ones) and which stand regardless. A negative test may empty *this* table without leaving the patient with nothing, because the Standard-of-care table is on the same page and unaffected. Say that, and name the standard options by label. Never write a caveat whose plain reading is "you have no options left" when the case page carries a standard-of-care table saying otherwise.

**Negative-result fallback (always required).** The cross-cutting caveat MUST state what a negative test leaves standing, naming the rows rather than gesturing at them: *"If `<biomarker>` is negative, ranks `<n>`–`<m>` are foreclosed. `<Any ungated rows that remain, named>` still stand in this table, and the Standard-of-care table on this page is unaffected: `<name the standard options>`."* Never let the caveat read as "nothing is left" when a standard-of-care table sits on the same page.
...
7. **Cross-cutting caveat (read first).** A bold-titled section that names the **load-bearing concern that shapes every rank**. Examples: a non-confirmed biomarker that gates the lead trial; a resistance mechanism that dominates the picture; a structural confound in the evidence base; a guideline-fit gap. Write 2–4 sentences plus a bullet list of the concrete consequences. This section earns the reader's first 30 seconds of attention; it must reflect what was actually load-bearing in the board's deliberation, not a generic disclaimer.
   ...
   **For biomarker-gated cases, the cross-cutting caveat MUST follow this structured pattern:**
    ...
    - One bullet stating explicitly what a negative test forecloses **and what it leaves**: *"If `<biomarker>` is negative, ranks `<n>`–`<m>` are foreclosed. `<Named ungated rows>` still stand here, and the Standard-of-care table is unaffected: `<named standard options>`."*
...
9. **Top interventions.** This is the substantive body of the page. For each row in `recommendations.jsonl` ranked 1..N **with `status` in (`recommended`, `considered_with_caveats`)**, render a level-2 sub-section with this exact internal structure:
   ...
   **Single unified ranking — no Path A / Path B split.** When the case has biomarker gating, render the workup row as the first H2 ("## Rank 1. DLL3 IHC SP347 on tumor — diagnostic gate"), then the biomarker-conditional therapeutic ranks below it (rank 2, 3, ...). Each conditional rec gets the italic *"Conditional on `<biomarker_short>:positive`. Foreclosed if test is negative."* note immediately under the H2. The reader sees ONE ranked list for the non-standard-of-care options, with `access_route` distinguishing trial-only from off-label and compassionate-use. Approved and guideline-carried therapies render in the Standard-of-care table instead; the scope note says so explicitly and names them.

10. **Also considered — not ranked.** This section documents the rows you emit with a non-`none` `surfaced_reason` — options considered but not ranked as live top-tier choices (`unavailable` / `consolidated` / `thin_evidence` / `not_enrollable`). One bullet each: label + the reason in a short phrase. The renderer also flags these in the "Also considered" sub-table, so keep this prose brief. When every option was ranked, write *"None — every option surfaced by the search was ranked."* These rows are still **in** the table: `surfaced_reason` demotes a row within the ranking, it never removes it. Nothing is omitted from `recommendations.jsonl` for being weakly evidenced — only approved and guideline-carried therapies leave, and they leave for the Standard-of-care table, never into nothing.
11. **Ranked prioritization.** A 6-column at-a-glance summary table modeled on io-shieldbreak's ranked-prioritization layout. Columns in order:
    ...
    **Cell content rules:**
    ...
    - **Rank** — integer, matching `rank` on the row. Render workup rows (`scenario: "shared"`) on a separate sub-table above the ranked options when biomarker gating applies. Rows with a non-`none` `surfaced_reason` render in a third "Also considered — not ranked" sub-table (the renderer replaces their Rank cell with a Flag badge) — still give them a `rank` integer for a stable sort order.
