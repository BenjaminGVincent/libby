...
5. **Rank EVERY therapy with any evidence in ONE unified table, and flag biomarker gating.** The `recommendations.jsonl` you produce is **the** ranked therapeutic table for this case. It is the case's most important output — more important than any prose you write — and a therapy that is not in it is, for practical purposes, invisible. Rules:
   - **Emit a row for every therapy with any evidence behind it**, drawn from `trials.jsonl`, `clinical_evidence.jsonl`, `preclinical_evidence.jsonl`, `accessibility.jsonl`, `standard_of_care.jsonl` (when it exists) and the board positions. Not a shortlist, not only what survived, and **not only the investigational ones**. If a therapy was assessed anywhere in the dossier, it gets a ranked row. When in doubt, include it and let `status` / `surfaced_reason` carry your reservation — omission is the one failure mode this table cannot recover from.
   - **Completeness is checked, not trusted.** `check_pipeline.py` fails the case when a `standard_of_care.jsonl` option has no corresponding ranked row. If a caller's brief lists candidates, treat that list as a floor and never as the set; the dossier is the authority on what was assessed.
   ...
   - **Rank approved / guideline-carried therapies here too — never route them out.** A drug approved (or NCCN/ESMO/ASCO/ASH-carried) for a population including this patient belongs in this ranking on the same footing as an investigational one, and it usually belongs near the top, because approved-and-deliverable is a genuine advantage over trial-only. Set **`access_route`** from `accessibility.jsonl::access_status` (`standard_of_care` / `off_label_use` / `clinical_trial_only` / `compassionate_use` / `expanded_access_program` / `not_yet_accessible` / `unavailable`); that field, not the therapy's absence, is what tells a reader an option is already standard care. Where a row carries several access paths, use the one this patient would actually take.
   - **Non-feature-targeting therapies are in scope for this table.** Surgery, radiotherapy, cytotoxic chemotherapy, palliative care and similar act on no stated molecular feature but are therapies with evidence, so they get ranked rows like anything else. This reverses the earlier routing rule: chemotherapy that the board ranked first must appear at the top of this table, not only on the standard-of-care page. **Referral pathways and enrollment strategies are routes, not therapies** — but they still need a row, because the coverage gate requires one for every `standard_of_care.jsonl` option and because a reader looking for "sarcoma-centre referral" should find it in the table rather than only in prose. Give them `surfaced_reason: "consolidated"` and point the row at the ranked therapies they deliver. That keeps them visible without spending ranked space that belongs to treatments, and the renderer files them under "Also considered — not ranked."
   - **`access_route: standard_of_care` is not a demotion.** Do not down-rank a therapy because it is standard care, and do not up-rank it merely because it is deliverable. Rank on the board's assessment of expected benefit, toxicity and fit, exactly as for any other row.
...
**Scope of the ranking.** This table ranks **every therapy with any evidence behind it** for this patient — investigational and approved, feature-targeting and not. It surfaces the full considered set: top-tier ranked plus non-top-tier flagged via `surfaced_reason`, so the reader sees everything the board weighed in one place. The `access_route` column distinguishes standard care from trial-only and off-label; nothing is withheld from the table on the basis of being approved. The workup row (`scenario: "shared"`) and the biomarker-conditional recs remain part of this table. The Standard-of-care page still exists and is still owned by `standard_of_care_screener`, but it is now a **detail surface** — regulatory status, guideline citations, eligibility and sequencing for the standard options — rather than a place therapies are moved *to*. Every therapy on it must also have a row here.
...
**The cross-cutting caveat in `index.md` carries the negative-branch mapping.** Do NOT enumerate the negative branch as a separate ranking. Instead, document in the cross-cutting caveat which rows a negative test forecloses (the `<biomarker>:positive` ones) and which stand regardless. Under the unified-table contract a negative test does **not** empty the table: the standard-of-care rows, and any option not gated on that biomarker, remain ranked and available. Name what is left, rather than sending the reader away to a separate conversation.

**Negative-result fallback (always required).** The cross-cutting caveat MUST state what a negative test leaves standing, naming the rows rather than gesturing at them: *"If `<biomarker>` is negative, ranks `<n>`–`<m>` are foreclosed. The remainder of the table stands, including `<the standard-of-care options and any ungated rows, named>`."* Only when a negative test genuinely leaves nothing may you say so, and that should be rare now that standard care is ranked in this table.
...
7. **Cross-cutting caveat (read first).** A bold-titled section that names the **load-bearing concern that shapes every rank**. Examples: a non-confirmed biomarker that gates the lead trial; a resistance mechanism that dominates the picture; a structural confound in the evidence base; a guideline-fit gap. Write 2–4 sentences plus a bullet list of the concrete consequences. This section earns the reader's first 30 seconds of attention; it must reflect what was actually load-bearing in the board's deliberation, not a generic disclaimer.
   ...
   **For biomarker-gated cases, the cross-cutting caveat MUST follow this structured pattern:**
    ...
    - One bullet stating explicitly what a negative test forecloses **and what it leaves**: *"If `<biomarker>` is negative, ranks `<n>`–`<m>` are foreclosed. The remainder of the table stands, including `<named standard-of-care and ungated options>`."*
...
9. **Top interventions.** This is the substantive body of the page. For each row in `recommendations.jsonl` ranked 1..N **with `status` in (`recommended`, `considered_with_caveats`)**, render a level-2 sub-section with this exact internal structure:
   ...
   **Single unified ranking — no Path A / Path B split.** When the case has biomarker gating, render the workup row as the first H2 ("## Rank 1. DLL3 IHC SP347 on tumor — diagnostic gate"), then the biomarker-conditional therapeutic ranks below it (rank 2, 3, ...). Each conditional rec gets the italic *"Conditional on `<biomarker_short>:positive`. Foreclosed if test is negative."* note immediately under the H2. The reader sees ONE ranked list covering every therapy with evidence — approved and investigational, feature-targeting and not — with `access_route` carrying the distinction.

10. **Also considered — not ranked.** This section documents the rows you emit with a non-`none` `surfaced_reason` — options considered but not ranked as live top-tier choices (`unavailable` / `consolidated` / `thin_evidence` / `not_enrollable`). One bullet each: label + the reason in a short phrase. The renderer also flags these in the "Also considered" sub-table, so keep this prose brief. When every option was ranked, write *"None — every option surfaced by the search was ranked."* Note that these rows are still **in** the table: `surfaced_reason` demotes a row within the ranking, it never removes it. Nothing is omitted from `recommendations.jsonl` on the grounds of being approved, non-targeting, or weakly evidenced.
11. **Ranked prioritization.** A 6-column at-a-glance summary table modeled on io-shieldbreak's ranked-prioritization layout. Columns in order:
    ...
    **Cell content rules:**
    ...
    - **Rank** — integer, matching `rank` on the row. Render workup rows (`scenario: "shared"`) on a separate sub-table above the unified ranked options when biomarker gating applies. Rows with a non-`none` `surfaced_reason` render in a third "Also considered — not ranked" sub-table (the renderer replaces their Rank cell with a Flag badge) — still give them a `rank` integer for a stable sort order.
