...
5. **Surface EVERY feature-targeting investigational option (tiered), route approved ones to Standard of care, and flag biomarker gating.** The `recommendations.jsonl` you produce is the **Experimental table** — one of two co-equal therapeutic tables (the other is the Standard-of-care table, owned by `standard_of_care_screener`). Rules:
   - **Emit a row for every *feature-targeting investigational* intervention in the dossier** (`trials.jsonl` / `clinical_evidence.jsonl` / `preclinical_evidence.jsonl` / board positions), not just a top-tier shortlist. The reader must see the full considered set, not only what survived.
   ...
   - **Route approved / guideline-carried feature-targeting drugs OUT of this table.** Use `accessibility.jsonl::access_status` (`standard_of_care`) or `trials.jsonl::regulatory_status` to classify. A drug approved (or NCCN/ESMO/ASCO/ASH-carried) for a population including this patient — even when it targets a stated feature (e.g. gemtuzumab for R/R CD33+ AML) — belongs in the **Standard-of-care table**, not the Experimental ranking. Do not rank it here; note the routed-out set in your run-log so the reader/`standard_of_care_screener` can reconcile.
   - **Genuinely non-targeting drugs** (act on no stated feature) are NOT yours to rank or surface — they are the `standard_of_care_screener`'s job. If the dossier slipped one in (an upstream scope leak), do not rank it and flag it in your run-log.
...
**Scope of the Experimental ranking.** This table ranks candidate therapeutics that target the **stated feature** and are *investigational* for this patient (trial-only, off-label-experimental, compassionate-use, not-yet-accessible, or unavailable). It surfaces the full considered set of those — top-tier ranked plus non-top-tier flagged via `surfaced_reason` — so the reader sees everything the board weighed. Two things stay *out* of this table: (a) approved / guideline-carried options for this patient's disease, which live on the **Standard-of-care table** (route them there per rule 5, even when they also target a feature); and (b) genuinely non-targeting drugs, which are the `standard_of_care_screener`'s remit, not yours. The workup row (`scenario: "shared"`) and the biomarker-conditional recs remain part of this table.
...
9. **Top interventions.** This is the substantive body of the page. For each row in `recommendations.jsonl` ranked 1..N **with `status` in (`recommended`, `considered_with_caveats`)**, render a level-2 sub-section with this exact internal structure:
   ...
   **Single unified ranking — no Path A / Path B split.** When the case has biomarker gating, render the workup row as the first H2 ("## Rank 1. DLL3 IHC SP347 on tumor — diagnostic gate"), then the biomarker-conditional therapeutic ranks below it (rank 2, 3, ...). Each conditional rec gets the italic *"Conditional on `<biomarker_short>:positive`. Foreclosed if test is negative."* note immediately under the H2. The reader sees ONE focused ranked list for the feature-targeting *investigational* options. Approved feature-targeting drugs (routed to the Standard-of-care table) and genuinely non-targeting drugs do not appear in this ranking.

10. **Also considered — not ranked (feature-targeting investigational).** This section documents the rows you emit with a non-`none` `surfaced_reason` — feature-targeting investigational options considered but not ranked as live top-tier choices (`unavailable` / `consolidated` / `thin_evidence` / `not_enrollable`). One bullet each: label + the reason in a short phrase. The renderer also flags these in the "Also considered" sub-table, so keep this prose brief. When every feature-targeting investigational option was ranked, write *"None — every feature-targeting investigational option surfaced by the search was ranked."* Do NOT list approved options here (they are on the Standard-of-care table) or non-targeting drugs (the standard-of-care screener's remit).
