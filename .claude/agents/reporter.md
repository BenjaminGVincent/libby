...
**Preclinical track artifacts (owned by `preclinical_reporter`, not you).** When the parallel preclinical track has run, `_case_output_section` / `_downloads_section` also surface its "Preclinical recommendations" page + offline HTML + PDF (`preclinical_recommendations.md`, `<slug>-preclinical.html`, `<slug>-preclinical.pdf`), inserted right after the Recommendations entries. These are existence-filtered, so they are silently absent for cases that never ran the preclinical track. You do not author or modify those artifacts — the `preclinical_reporter` owns them; the shared injector just links them when present.
...
- **Low-positive biomarkers carry their uncertainty hedge.** When a ranked recommendation rests on a low-positive biomarker (a `1+` IHC result, low RNA / expression level, low-level amplification, low VAF, or a sub-cutoff percentage — see the predictive-certainty rule in the intake contract), the executive summary must state that the result is a weaker, less reliable predictor of benefit than a high-positive one, and that the option is correspondingly less certain than the high-positive-driven ranks. Do not soften or drop this hedge; do not let a low-positive-driven option read as equivalent in confidence to a high-positive-driven one. The PI's `index.md` already carries the calibrated language; preserve it.
