...
2. **Enumerate by modality cross-product.** Run a separate search for each cell of `<feature> × {BiTE, bispecific antibody, trispecific antibody, ADC, antibody-drug conjugate, radioligand, radioimmunotherapy, radioconjugate, theranostic, CAR-T, CAR-NK, small molecule, vaccine}`. Each modality surfaces a different slice of the pipeline; a flat `<feature>` query merges them and biases toward the most-published modality.

    **Radiopharmaceuticals need an isotope sweep, not just a modality word.** These programs are often registered under an isotope-bracketed name (`[177Lu]Lu-DOTA-<antibody>`) with no modality term in the title, so the words above miss them. Add `<feature> × {177Lu, lutetium, 225Ac, actinium, 131I, iodine-131, 90Y, 211At, 212Pb, 227Th}` as its own axis, run it across all recruitment statuses, and record the isotope sweep in `search.md` so a later reader can see it ran. Classify what it returns by the radiopharmaceutical rule in Step 2 — do not pre-filter on the assumption that a radiolabeled agent is an imaging tracer.
...
- **Keep — radioligand / radioconjugate with therapeutic intent:** a radiolabeled agent directed at one of the patient's targetable features is a **therapy row**, ranked and screened like any ADC or bispecific. Set `modality: radioligand` (or `radioimmunotherapy`) and the `tumor_type_relationship` the trial's own indication warrants. See the radiopharmaceutical rule below — this is the one modality that gets systematically misfiled as a diagnostic.
- **Drop:** reviews, editorials, meta-analyses (unless user opts in), preclinical-only papers, **imaging-only studies with no therapeutic administration** (see the radiopharmaceutical rule below for the boundary), **and any trial whose drug does not plausibly target one of the patient's targetable features — even if the trial enrolls the patient's tumor type at the right line of therapy.** Standard 2L+ care for the indication that does not target the feature is out of scope; the patient pursues those through their treating team independent of Libby. **This includes a drug approved (or in trials) for the patient's tumor type whose eligibility hinges on a molecular alteration the patient does NOT have** — e.g. a menin inhibitor (targets KMT2A-rearranged / NPM1-mutant AML) in a patient who is NPM1-wild-type with KMT2A *amplification*. It is an approved AML drug, but it does not act on any *stated targetable feature* of this patient, so it does not belong in `trials.jsonl`. If it is standard/approved for the disease it will be surfaced by the `standard_of_care_screener` (flagged "target absent"); do not route it into the feature-targeting dossier.

**Radiopharmaceutical rule — therapeutic intent decides, not the molecular class.**

A radiolabeled agent against a targetable feature is a *therapy* unless the study administers no therapeutic dose at all. Classify on what the protocol does to the patient, never on the word "radio-", "tracer", "PET", or "scan" appearing in the title.

- **Keep — therapeutic radioligand.** Any study delivering a cytotoxic radionuclide to a target on the feature list: `177Lu`, `225Ac`, `131I`, `90Y`, `211At`, `212Pb`, `227Th`, `161Tb`. Screen it against the patient's biomarker exactly as you would an ADC.
- **Keep — theranostic pair.** A protocol pairing a diagnostic tracer with a therapeutic dose (`68Ga` imaging gating `177Lu` treatment, or a `68Ga`/`131I` pair) is a therapy trial. The imaging arm is a selection gate, not the purpose. A dosimetry-only primary endpoint does not make it diagnostic if a therapeutic isotope is administered — record the limitation in `inclusion_match_notes` and keep the row.
- **Drop — imaging-only.** Studies administering *only* a diagnostic isotope (`68Ga`, `18F`, `64Cu`, `89Zr`, `99mTc`) with no therapeutic arm anywhere in the protocol: immunoPET, biodistribution, and staging studies. These belong nowhere in `trials.jsonl`.
- `131I` is both diagnostic and therapeutic depending on activity and intent. Read the protocol; do not infer from the isotope alone.

**Never drop a modality class wholesale.** If a search axis returns hits you judge out of scope, that judgement is per-trial, and the reasoning goes in the run log. A class-level exclusion is exactly how the failure below happened, and it is invisible afterward because the dropped rows leave no trace.

> **Failure this rule fixes.** In `pdac-recurrent-cldn18-2-kras-g12r-tp53-y220c-p25m`, the whole CLDN18.2 radioligand class was bucketed with imaging-only tracers and dropped. The horizon-scan agent later surfaced `NCT07595237` incidentally. It was verified recruiting, named pancreatic cancer, and set a cutoff of CLDN18.2 ≥2+ in ≥20% of cells — which the patient's 2+/3+ in 60% cleared threefold, against a tumor that fell *below* the zolbetuximab antibody cutoff. It graded `fit_to_case: strong` and reached the board only as a late correction, after all five positions were written.

That case also shows *why* this class matters disproportionately: a radioligand kills through crossfire rather than requiring antigen engagement on every cell, so it is most valuable exactly where heterogeneous or sub-threshold expression makes antibodies, ADCs, and CAR products marginal. The patient whose IHC just missed an antibody cutoff is the patient for whom this class is most relevant — and the one for whom silently dropping it does the most damage.
